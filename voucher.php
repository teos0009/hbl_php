...
//http://164.78.200.43/~teosj/hbl/voucher.php?a=123
	...
	//if(isset($_COOKIE['hash']) && $_COOKIE['hash'] == md5($vnum.$password.$date)) {//invuln
	if(isset($_COOKIE['hash']) && $_COOKIE['hash'] == md5($vnum.$date.$password)) {//vuln
		if($date < date('Ymd')) {
			$vnum++;
	...
	//setcookie("hash", md5($vnum.$password.$date), time()+$cookielife);//invuln
	setcookie("hash", md5($vnum.$date.$password), time()+$cookielife);//vuln
